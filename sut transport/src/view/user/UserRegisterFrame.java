...
public class UserRegisterFrame extends JFrame {
    ...
    public UserRegisterFrame(SystemManager manager) {
        this.systemManager = manager;

        setTitle("SUT-TRANSPORT - สมัครสมาชิก");
        ...
        panel.setBounds(210, 80, 480, 470);
        panel.setLayout(null);
        panel.setBackground(new Color(255, 175, 80)); 
        getContentPane().add(panel);

        JLabel lblTitle = new JLabel("สมัครสมาชิก", SwingConstants.CENTER);
        ...
        createLabel(panel, "ชื่อผู้ใช้ :", startY);
        txtName = createField(panel, startY + 30);

        JLabel lblStudentId = new JLabel("รหัสนักศึกษา :");
        try { lblStudentId.setFont(FontManager.bold(14f)); } catch(Exception e) {}
        lblStudentId.setForeground(Color.WHITE);
        lblStudentId.setBounds(70, startY + gap, 150, 25);
        panel.add(lblStudentId);
        
        txtStudentId = new JTextField();
        setupFieldStyle(txtStudentId);
        RoundedPanel boxId = createInputBox();
        boxId.setBounds(70, startY + gap + 30, 160, 35);
        boxId.setLayout(null);
        txtStudentId.setBounds(10, 5, 140, 25);
        boxId.add(txtStudentId);
        panel.add(boxId);

        JLabel lblEmail = new JLabel("Email :");
        try { lblEmail.setFont(FontManager.bold(14f)); } catch(Exception e) {}
        lblEmail.setForeground(Color.WHITE);
        lblEmail.setBounds(250, startY + gap, 150, 25);
        panel.add(lblEmail);

        txtEmail = new JTextField();
        setupFieldStyle(txtEmail);
        RoundedPanel boxEmail = createInputBox();
        boxEmail.setBounds(250, startY + gap + 30, 160, 35);
        boxEmail.setLayout(null);
        txtEmail.setBounds(10, 5, 140, 25);
        boxEmail.add(txtEmail);
        panel.add(boxEmail);

        createLabel(panel, "รหัสผ่าน :", startY + (gap * 2));
        RoundedPanel passBox = createInputBox();
        passBox.setBounds(70, startY + (gap * 2) + 30, 340, 35);
        ...
        btnRegister.addActionListener(e -> {
            ...
            if (systemManager.registerPassenger(name, email, pass, id)) {
                showCustomDialog("สำเร็จ", "สมัครสมาชิกสำเร็จ!", "success");
                dispose();
                SwingUtilities.invokeLater(() -> new UserLoginFrame(this.systemManager).setVisible(true));
            } else {
                showCustomDialog("ผิดพลาด", "สมัครไม่สำเร็จ (ข้อมูลซ้ำ)", "error");
            }
        });
    }
    ...
    private void createLabel(JPanel p, String text, int y) {
        ...
        lbl.setBounds(70, y, 150, 25);
        p.add(lbl);
    }
    
    private void setupFieldStyle(JTextField txt) {
        txt.setBorder(null);
        try { txt.setFont(FontManager.regular(14f)); } catch(Exception e) {}
    }

    private JTextField createField(JPanel p, int y) {
        JTextField txt = new JTextField();
        setupFieldStyle(txt);
        RoundedPanel box = createInputBox();
        box.setBounds(70, y, 340, 35);
        box.setLayout(null);
        txt.setBounds(10, 5, 320, 25);
        box.add(txt);
        p.add(box);
        return txt;
    }
    ...
    private JButton createCustomBackButton() {
        ...
        btnBack.addActionListener(e -> {
            dispose();
            new UserLoginFrame(this.systemManager).setVisible(true);
        });
        return btnBack;
    }
